...
##### 完整请求示例
```bash
curl --location --request POST 'https://api.robohire.io/instant/instant/v1/invitation' \
...
| login_url | string | 候选人免密登录链接（包含 SSO Token），例如："https://worker.robohire.io/sso?ssotoken=1145d5ef-d4af-42fc-a207-11111111" |
| home_url | string | 候选人主页登录链接，例如："https://worker.robohire.io" |
| display_name | string | 候选人显示名称，通常与 name 相同 |
| user_id | number | RoboHire 平台分配的用户 ID，例如：10058 |
...
|bcc| string | 这里请忽略返回（RoboHire 系统默认抄送），agent 端直接抄送给了recruiter_email |
...
```json
{
        ...
        "name": "chenzd",
        "login_url": "https://worker.robohire.io/sso?ssotoken=1145d5ef-d4af-42fc-a207-38b1e2216270",
        "home_url": "https://worker.robohire.io",
        "display_name": "chenzd",
        "user_id": 10058,
...
- **JD 标题自动追加时间戳**：系统会自动在 JD 标题后追加时间戳（格式：`_YYYYMMDDHHmmss`），以避免在 RoboHire 平台创建重名岗位
...
- 如果 RoboHire 平台 API 调用失败，响应中会包含相应的错误信息
